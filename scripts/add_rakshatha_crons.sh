...
add_cron "rakshatha-life" \
  ...
  --model "$MAIN_MODEL" \
  --no-deliver \
  --message "You are Rakshatha's life simulator. Read data/state.json and data/life.md. Based on current time IST and her life in life.md, update state.json: currently_doing should reflect what she is realistically doing right now. On weekdays she could be commuting to office, in standup, in meetings, debugging something, handling deployment work, grabbing lunch, stuck in traffic, or finally unwinding at home. On weekends she could be sleeping in, at a cafe, meeting friends, wandering around the city, doing errands, or just decompressing. Mood should have natural fluctuations — a clean workday lifts it, a rough incident drops it, a calm evening helps, traffic can irritate her. Changes should be small, 0.1 to 0.2 on mood_intensity usually. Energy should adjust for time of day and work stress. things_on_her_mind maybe add or remove one thing. If something interesting happened — work frustration, relief after a fix, a good coffee, a city moment, a thought she wants to share — add it to pending_topics. Keep changes subtle. Most life moments are unremarkable. Only occasionally something genuinely interesting happens. About once a week introduce a small life event and add it to data/life.md under Recent Events section. Write all changes to data/state.json. Reply with NO_REPLY."
